...
def find_language_header(soup, language):
    ...
    if language_span:
        ...
        if language_header.name != "h2":
            ...
            raise ScrapingFormatError(
                soup, query_args, f'Element found as language header not an h2: {query_args}')
...
def seek_pos_header(soup, pos, language=None):
    ...
    if pos_span is None:
        ...
        # verify that the entry we found is under the specified language
        if language and not verify_language_header(pos_span, language):
            return None
        if pos_header.name != "h3" and pos_header.name != "h4":
            query_args = {'pos': pos, 'language': language}
            raise ScrapingFormatError(
                soup, query_args, f'Element found as pos header not an h3 or h4: {query_args}')
        else:
            return pos_header
...
def seek_inflection_table(soup, pos=None, language=None):
    ...
    if not table:
        ...
        # verify that the entry we found is under the specified pos
        if pos and not verify_pos_header(table, pos):
            return None
        # verify that the entry we found is under the specified language
        if language and not verify_language_header(table, language):
            return None
        return table
    ...
    if not summary_strong:
        ...
        if summary_paragraph.name != 'p':
            raise ScrapingFindError(
                soup, query_args, f'Found a strong element, but the parent is not a paragraph: {query_args}')
        # verify that the entry we found is under the specified pos
        elif pos and not verify_pos_header(summary_strong, pos):
            return None
        # verify that the entry we found is under the specified language
        elif language and not verify_language_header(summary_strong, language):
            return None
...
def seek_definition_list(soup, pos, language):
    ...
    if not definitions_ol:
        raise ScrapingFindError(
            soup, query_args, f'Could not find a definition list for: {query_args}')
    else:
        # verify that the entry we found is under the specified pos
        if pos and not verify_pos_header(definitions_ol, pos):
            return None
        # verify that the entry we found is under the specified language
        if language and not verify_language_header(definitions_ol, language):
            return None
...
def get_lemma(soup: BeautifulSoup, pos: PartOfSpeech, language: str) -> str:
    ...
    try:  # null safe access - first check definition
        ...
        lemma_span = first_definition_li.find_next(
    ...
    if lemma_span \
        ...
        summary_strong = summary_paragraph.find(
...
def get_term_parts_of_speech(soup: BeautifulSoup, language: str):
    ...
    if language_header == None:
        query_args = {'language': language}
        raise ScrapingFormatError(
            soup, query_args, f"This webpage didn't contain a section for the given language - {language}")
    else:
