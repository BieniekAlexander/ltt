...
def extract_lexeme(soup, lemma, pos, language):
    ...
    if pos.lower() == "adjective":
        ...
        raise ValueError(
            f"Tried to extract an unknown part of speech: {pos.lower()}")
...
def parse_features_noun(soup, pos, language):
    ...
    gender_span = lemma_summary_paragraph.find_next(
        'span', {'class': 'gender'})

    if not gender_span:
        raise ScrapingFindError(
            lemma_summary_paragraph, {}, "Could not find gender information for this noun")
    else:
        for abbr in gender_span.find_all("abbr"):
            ...
            if value == 'nonvirile':
                ...
                logging.warn(
                    f"skipping parsing of noun data for class value={value}")
            else:
                query_args = {'pos': pos, 'language': language}
                raise ScrapingValueError(
                    gender_span, query_args, 'class value', value)
        ...
        for section in sections:
            for form in forms:  # check the section for form labels and parse out the forms
                if form in section:
                    ...
                    entry_strings = re.split(
                        r' or |,', entries_string)  # split out entries
                    # remove empty strings
                    entries_raw = list(
                        filter(lambda x: x.strip() != '', entry_strings))
                    # clean entry strings
                    entries = list(map(lambda x: x.strip(), entries_raw))
                    ret[form] = entries
                    break
            else:
                query_args = {'pos': pos, 'language': language}
                raise ScrapingValueError(lemma_summary_paragraph, query_args, None, None,
                                         "This noun summary section contained a property that we're not accounting for")
...
def parse_features_verb(soup, pos, language):
    ...
    aspect_span = lemma_summary_paragraph.find_next(
        'span', {'class': 'gender'})  # html seems mislabeled, notice class=gender

    if not aspect_span:
        raise ScrapingFindError(
            lemma_summary_paragraph, {}, "Could not find aspect information for this verb")
    else:
        for abbr in aspect_span.find_all("abbr"):
            ...
            if value == 'imperfective aspect':
                ...
                query_args = {'pos': pos, 'language': language}
                raise ScrapingValueError(
                    aspect_span, query_args, 'aspect', value)

    # get abstraction, alternate forms
    forms_match = re.search(r'\(.*\)', summary_string)
    forms = ['perfective', 'imperfective', 'indeterminate',
             'imperfective determinate', 'frequentative', 'determinate', '+']
            ...
            for form in forms:  # check the section for form labels and parse out the forms
                if form in section:
                    ...
                    if form == '+':  # TODO some verb sections describe associated cases, skipping this for now
                        logging.warn(
                            f"Skpping parsing of this verb case usage summary section - {section}")
                    ...
                    # this section describes alternative forms (e.g. "perfective zjeść")
                    else:
                        entry_strings = re.split(
                            r' or |,', entries_string)  # split out entries
                        # remove empty strings
                ...
                query_args = {'pos': pos, 'language': language}
                raise ScrapingValueError(lemma_summary_paragraph, query_args, None, section,
                                         f"This verb summary section contained a property that we're not accounting for - {section}")
...
def parse_features_adjective(soup, pos, language):
    ...
    # if the adjective describes other forms (or lack thereof), it's usually the positive form
    if forms_match:
        ret['degree'] = 'positive'
        forms_string = forms_match.group(0)[1:-1]
        sections = list(map(lambda x: x.strip(), forms_string.split(',')))
        ...
        for section in sections:
            for form in forms:  # check the section for form labels and parse out the forms
                if form in section:
                    ...
                    # skipping weird edge cases in the summary section
                    if entries_string.strip() in ['not always comparable']:
                        pass
                    elif entries_string.strip() == "" and form != "not comparable":
                        query_args = {'pos': pos, 'language': language}
                        raise ScrapingValueError(lemma_summary_paragraph, query_args, None, section,
                                                 f"This adjective summary section contained a property that we're not accounting for - {section}")
                    elif form == "not comparable":
                        ret['not_comparable'] = True
                    # this section describes alternative forms (e.g. "comparative czerwieńszy or bardziej czerwony")
                    else:
                        entry_strings = re.split(
                            r' or |,', entries_string)  # split out entries
                        # remove empty strings
    ...
    # the comparative and superlative entries don't have summaries - they instead list degree in the definition (e.g. "comparative degree of czerwony")
    else:
        ...
        if 'comparative degree' in degree_definition:
            positive = re.match(
                r'comparative degree of ([a-zA-Z]*)', degree_definition).group(1)
            ret['degree'] = 'comparative'
            ret['positive'] = [positive]
        elif 'superlative degree' in degree_definition:
            positive = re.match(
                r'superlative degree of ([a-zA-Z]*)', degree_definition).group(1)
            ret['degree'] = 'superlative'
            ret['positive'] = [positive]
        else:
            args = {'pos': pos, 'language': language}
            raise ScrapingFindError(
                soup, args, "Expected description of non-positive degreed adjective, failed to parse such information")
...
def parse_features_adverb(soup, pos, language):
    ...
    # if the adverb describes other forms (or lack thereof), it's usually the positive form
    if forms_match:
        ret['degree'] = 'positive'
        forms_string = forms_match.group(0)[1:-1]
        sections = list(map(lambda x: x.strip(), forms_string.split(',')))
            ...
            for form in forms:  # check the section for form labels and parse out the forms
                if form in section:
                    ...
                    if entries_string.strip() == "" and form != "not comparable":
                        query_args = {'pos': pos, 'language': language}
                        raise ScrapingValueError(lemma_summary_paragraph, query_args, None, section,
                                                 f"This adverb summary section contained a property that we're not accounting for - {section}")
                    elif form == "not comparable":
                        ret['not_comparable'] = True
                    # this section describes alternative forms (e.g. "comparative czerwieńszy or bardziej czerwony")
                    else:
    ...
    # the comparative and superlative entries don't have summaries - they instead list degree in the definition (e.g. "comparative degree of szybko")
    else:
        # https://stackoverflow.com/a/42536557
        degree_definition = definitions[0]
