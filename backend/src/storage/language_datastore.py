...
class LanguageDatastore(object):
    ...
    def __init__(self, datastore_client: MongoClient, language: str):
        ...
        self.inflections_connector = InflectionsConnector(
            datastore_client, language)
    ...
    def add_lexeme(self, lexeme: Lexeme):
        ...
        if isinstance(lexeme, InflectedLexeme):
            ...
            entries = [{'lexeme_id': lexeme_id, 'pos': lexeme.pos,
                        'form': form} for form in inflections]
            self.inflections_connector.push_inflection_entries(entries)
        else:
            self.inflections_connector.push_inflection_entry(
                lexeme_id, lexeme.lemma, lexeme.pos)
    ...
    def get_lexeme_from_form(self, form: str, pos: str):
        ...
        assert isinstance(pos, str) and pos in [
            pos.value for pos in PartOfSpeech]

        entry = self.inflections_connector.get_inflection_entry(
            form=form, pos=pos)
    ...
    def get_lexemes_from_form(self, form: str, poses: list = None) -> dict:
        ...
        if poses == None:
            poses = []
        assert isinstance(form, str)
        assert all(isinstance(pos, str) and pos in [
                   pos.value for pos in PartOfSpeech] for pos in poses)

        # TODO restore
        inflection_entries = self.inflections_connector.get_inflection_entries(forms=[
                                                                               form], poses=poses)
        lexeme_ids = [ObjectId(d['lexeme_id']) for d in inflection_entries]

        if lexeme_ids:
            lexeme_dicts_dict = self.lexicon_connector.get_lexeme_entries(
                _ids=lexeme_ids)
    ...
    def add_vocabulary_entry(self, lexeme_id: str, stats: dict, user_id: str) -> str:
        ...
        """
        self.vocabulary_connector.push_vocabulary_entry(
